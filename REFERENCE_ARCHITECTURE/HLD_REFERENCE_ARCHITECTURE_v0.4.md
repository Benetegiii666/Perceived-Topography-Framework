# HLD / Reference Architecture v0.4 — Pre-QA Refinement

**Date:** 2026-06-22
**Status:** v0.4 Pre-QA Refinement. Targeted schema and authority corrections applied. No structural changes.
**Classification:** HLD v0.4 Pre-QA Refinement
**Base:** v0.3 Complete Draft (2026-06-22)
...
| `reasoning_status` | enum | Draft / Provisional / Validated | Governs whether the reasoning behind this decision has been confirmed or validated. Follows the same lifecycle semantics as all other reasoning objects (Principle 7). |
| `decision_status` | enum | Active / Archived / Superseded | Governs whether this decision/action is currently active, historical, or replaced by a later decision. |
| `created_at` | timestamp | When the decision was made |

**Why two status fields:** A decision can be Active (the action is still in effect) while its reasoning is only Provisional (not yet validated by outcome evidence). Conversely, a decision can be Archived (the campaign ended) while its reasoning remains Validated and useful for future learning and retrieval. Collapsing these into a single field conflates governance of the reasoning with the operational state of the decision.
...
- `reasoning_status`: Draft → Provisional → Validated (same as all reasoning objects; governed by Chapter 8 lifecycle rules)
- `decision_status`: Active (current decision), Archived (after outcome captured), Superseded (if a later decision replaced it)
...
| `authority` | enum | binding / advisory / background / anecdotal | Trust level of this source, aligned with the Build Discovery Guide trust model. See Authority Taxonomy below. |
...
**Authority Taxonomy** (aligned with Build Discovery Guide trust model):

| Authority Level | Definition | Retrieval / Governance Implications |
|---|---|---|
| **binding** | Governs behavior or output. Must be followed unless formally overridden (e.g., compliance policy, approved claims list, legal guidance). | Retrieval: surfaces with high priority. Governance: violations trigger escalation or blocking. Premises citing binding sources carry higher initial confidence. |
| **advisory** | Useful and trusted, but not governing (e.g., best-practice guide, senior leader recommendation, validated playbook). | Retrieval: surfaces alongside binding sources. Governance: departures should be explained but do not require formal override. |
| **background** | Context only. Informs interpretation but does not justify claims (e.g., market research, historical campaign data, industry report). | Retrieval: surfaces as supporting context. Governance: cannot be sole basis for policy-level premises. |
| **anecdotal** | Signal only. May inform investigation but cannot govern output (e.g., single sales conversation, informal feedback, one customer complaint). | Retrieval: surfaces with "anecdotal — investigate before relying" flag. Governance: premises built solely on anecdotal sources must be flagged as Low confidence. |

This taxonomy feeds retrieval weighting (binding sources rank higher), provenance display (the human sees the authority level alongside the source), and governance rules (premises citing only anecdotal sources are flagged for additional investigation).
...
|---|---|---|
| **Requester / Operator** | The person who initiates a workflow and interacts with Discovery. Creates Draft reasoning objects. Confirms proposed reasoning within their own workflow, limited to intent and local workflow context (see Requester Confirmation Scope below). | Marketer, support agent, analyst, campaign manager, product manager |
...
#### Requester Confirmation Scope

Requester confirmation (Draft → Provisional) is valid for:

- Their own intent ("yes, the goal is qualified pipeline, not awareness")
- Workflow request context ("this campaign targets cardiology administrators")
- Goal interpretation ("the system correctly understood my request")
- Local facts they personally own ("we need this by end of quarter")
- Whether the system understood the request correctly

Requester confirmation is **not sufficient** to validate:

- Domain claims ("workflow burden predicts buying readiness")
- Compliance or policy claims ("this language is approved for healthcare")
- Evidence strength ("this data supports the premise")
- Policy boundaries ("this claim does not cross the compliance threshold")
- Cross-workflow applicability ("this lesson applies to oncology campaigns too")
- Reusable MUOs ("this model update should shape all future campaigns")
- High-consequence premises whose failure could cause material harm

Unless the Requester also holds the relevant Domain Expert or Governance Owner role for the domain in question.

Requester confirmation is valid for intent and local workflow context. It is not sufficient to validate domain, policy, compliance, evidence, or reusable learning claims unless the Requester also holds the relevant authority scope.
...
| **Decision State** | id, optimizer_state_id, chosen_action, rejected_alternatives, sufficiency_rationale (required), confidence_at_decision, unresolved_uncertainties, reasoning_status, decision_status, created_at |
...
- Full HLD drafted: **Yes** (v0.4 — all 16 chapters drafted. Pre-QA refinements: Decision State dual-status model, Requester confirmation scope, Information Surface authority taxonomy.)
