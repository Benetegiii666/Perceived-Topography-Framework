...
**A gradient** is a directional pressure within that environment. It is the reason one path feels easier, more attractive, more available, or more sufficient than another. A persuasive but unsupported claim has a strong gradient because it pulls attention toward action. A disconnected policy has a weak gradient because it exists without shaping the decision.
...
**Learning as model change** means that learning is not storing what happened. Learning is changing the reasoning that would otherwise reproduce the same error. A campaign report is not a lesson. A lesson is an evidence-supported change to how the system reasons about the next similar decision — with scope, confidence, and boundaries so the lesson does not get overapplied.

---

## Why Prompt Libraries and Context Stuffing Are Not Enough

Prompt libraries help teams ask better questions. They are worth building. But a better prompt does not automatically preserve why a prior decision worked, why it failed, or what should change the next time.

Retrieval-augmented generation (RAG) helps systems access more information. It is a necessary infrastructure layer. But retrieving documents is not the same as retrieving reasoning. A system can retrieve a prior campaign brief without knowing which premise was tested, which assumption failed, which evidence boundary mattered, or whether the prior lesson applies to the current situation.

Neither prompt libraries nor RAG preserve the reasoning transition: the movement from goal and interpretation, through decision and action, to outcome and update. Without that transition, the next cycle starts from the same shallow place.
...
- **Track what changed after outcomes arrive.** When reality contradicts expectations, the organization should produce a specific, bounded update — not a vague lesson. "Improve targeting" is not learning. "Workflow pain attracts attention but does not reliably indicate buying readiness — distinguish attention from qualification" is learning.
...
- Discovery replaces blank-form intake — the system infers, proposes, and the human confirms.
...
*Draft status: v0.1 companion artifact. Derived from PAPER_v1.0_WORKING.md and companion artifact planning notes. Not a replacement for the full paper.*
